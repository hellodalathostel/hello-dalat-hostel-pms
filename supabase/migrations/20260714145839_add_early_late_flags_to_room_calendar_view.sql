-- Them has_early_check_in, has_late_check_out vao view room_calendar de
-- Calendar UI hien thi icon dong ho phan biet booking co early/late.
-- Nhanh bookings: lay truc tiep tu bang. Nhanh room_blocks/ota_block: false co dinh
-- vi khong ap dung early/late cho block.
CREATE OR REPLACE VIEW public.room_calendar AS
 SELECT b.room_id,
    ...
    b.has_early_check_in,
    b.has_late_check_out
   FROM bookings b
...
UNION ALL
 SELECT rb.room_id,
    ...
    false AS has_early_check_in,
    false AS has_late_check_out
   FROM room_blocks rb
