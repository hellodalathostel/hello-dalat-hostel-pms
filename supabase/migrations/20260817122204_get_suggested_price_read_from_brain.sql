...
-- Migration nay DA APPLY tren production tu 17/08/2026.
...
DECLARE
  ...
  v_isodow        INTEGER := EXTRACT(ISODOW FROM p_date)::INTEGER; -- 1=Mon ... 7=Sun
  v_is_peak       BOOLEAN;
  v_is_weekend    BOOLEAN;
  v_factor        NUMERIC := 0.4475; -- cong_thuc_standard_rate
  v_holidays_raw  TEXT;
...
BEGIN
  ...
  -- 2. Lấy Standard Rate + Net mục tiêu từ Brain
  ...
  v_net := (regexp_match(v_raw, 'Net mục tiêu:\s*([\d,]+)'))[1]::TEXT;
  v_net := replace(v_net, ',', '')::NUMERIC;

  v_standard_rate := (regexp_match(v_raw, 'Standard Rate:\s*([\d,]+)'))[1]::TEXT;
  v_standard_rate := replace(v_standard_rate, ',', '')::NUMERIC;
  ...
  IF v_is_holiday THEN
    ...
    RETURN ROUND(v_net / v_factor);
  END IF;

  -- 5. Ngày thường: season × weekend nhân dồn lên Standard Rate
  v_is_peak    := v_month IN (10, 11, 12, 1, 2);
  v_is_weekend := v_isodow IN (5, 6, 7); -- Thứ 6, 7, Chủ nhật

  IF v_is_peak THEN
    v_standard_rate := v_standard_rate * 1.15;
  END IF;

  IF v_is_weekend THEN
    v_standard_rate := v_standard_rate * 1.05;
  END IF;

  RETURN ROUND(v_standard_rate);
END;
$function$;